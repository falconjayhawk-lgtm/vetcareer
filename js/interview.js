...
function renderInterview() {
  ...
  // Route to airline interview prep
  ...
  // ── Standard civilian interview prep below ────────────────────────
...
Return ONLY this JSON (no markdown, no extra text):
{
  ...
  "closingQuestions": "4 smart questions to ask, numbered list"
    ...
    let result;
    try { result = typeof extractJSON === 'function' ? extractJSON(raw) : JSON.parse(raw.replace(/```json|```/g,'').trim()); }
